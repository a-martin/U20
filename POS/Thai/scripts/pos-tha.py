...
def makeButton(buttonName, buttonText):
    ...
    text = visual.ImageStim(
        win,
        image=imageChemin+thai[buttonText].decode('utf-8')+'.png',
        name=buttonText, # important to extract str to store in csv
        pos=buttonPositions[buttonName]
    )
...
def getClick(mouse, buttons, cons, eng):
    ...
    while not clicked:
        # button is a tuple where 0 is shape obj and 1 is image obj
    ...
    return responseButton, responseText.name
...
def initializeTrial(displayText, buttonNames, buttonTexts):
    ...
    aTraduire = visual.ImageStim(
        win,
        image=imageChemin+'BIG/'+displayText+'.png',
        pos=(0,200)
    )
    
    aTraduire.setAutoDraw(True)
    aTraduire.draw()
    ...
    return mouse, buttons, cons, aTraduire
...
def doTrainingTrial(noun, modifier, nTrial):
    ...
    # prepare the Thai and the Thai' possibilities
    thaText = noun + '' + modifier
    ...
    mouse, buttons, cons, aTraduire = initializeTrial(
        displayText=thaText,
        buttonNames=['A', 'B'],
        buttonTexts=['-----'] * len(buttonTexts)
    )

    # make and play sound objects based on the Thai'
    stims = makePhrase([modifier, noun])
    for stim in stims:
        playStim(stim)
    ...
    # update button text from dashes to actual content (change image and name of obj)
    for n,button in enumerate(buttons):
        button[1].image = imageChemin+buttonTexts[n]+'.png'
        button[1].name = buttonTexts[n]
        
    # activate mouse
    mouse.setVisible(True)

    # recover response and response button
    responseButton, response = getClick(mouse, buttons, cons, aTraduire)
    

    # check to see that participant selected the prenominal variant
    if response == prenom:
        correct = 1
    else:
        correct = 0
...
def doTestTrial(nTrial, noun, modOuter, modInner=None):
    ...
    if type(modInner) is str:
        ...
        thaText = noun + modInner + modOuter
...
def doTest(trials):
    ...
    for i in trials.index[:5]: # trials.index
...
# This block proposes the experiment be made full screen or not
...
if plein_ecran:
    win = visual.Window(
        fullscr=True,
        allowGUI=False, # this disallows the participant from changing the window, whatever
        color="lightgrey",
        colorSpace="rgb",
        units="pix",
    )
